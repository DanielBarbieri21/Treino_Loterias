...
def backtest_modelo(_df_numeros=None, _criar_features_func=None, _gerar_numeros_func=None, n_testes: int = 60, janela_treino: int = 120):
    ...
    resultado["estatisticas"] = {
        ...
        "senas": sum(hit == 6 for hit in resultado["hits"]),
        "quinas": sum(hit == 5 for hit in resultado["hits"]),
        "quadras": sum(hit == 4 for hit in resultado["hits"]),
    }
    return resultado
...
def visualizar_resultados_backtesting(resultados, salvar: bool = True):
    ...
    ax.set_title(f"Mega-Sena | Estratégia {stats['estrategia']}")
    ax.set_xlabel("Sorteio avaliado")
    ax.set_ylabel("Acertos")
    ax.legend()
    ax.grid(alpha=0.3)
    if salvar:
        plt.savefig("backtesting_resultados_megasena.png", dpi=200, bbox_inches="tight")
    return fig
...
def gerar_relatorio_backtesting(resultados, salvar_txt: bool = True):
    ...
    relatorio = (
        ...
        f"Senas: {stats['senas']} | Quinas: {stats['quinas']} | Quadras: {stats['quadras']}\n"
    ...
    if salvar_txt:
        Path("relatorio_backtesting_megasena.txt").write_text(relatorio, encoding="utf-8")
    return relatorio
