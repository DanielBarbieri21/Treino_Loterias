# 🎯 Sistema Avançado de Análise e Previsão - Mega-Sena

## 📋 Visão Geral

Este sistema avançado foi desenvolvido para **identificar padrões profundos** e **aumentar a probabilidade de acerto** na Mega-Sena através de análises estatísticas robustas e machine learning.
...
- ✅ Gerar **6 números mais prováveis** baseado em múltiplos fatores
...
├── mega_sena.xlsx              # Base de dados históricos
└── modelo_avancado_mega.pkl     # Modelo treinado (gerado automaticamente)
```
...
Coloque seu arquivo `mega_sena.xlsx` na pasta `Mega_Sena/` com os sorteios históricos.
...
### 1. **Padrões Temporais**
- **Ciclos**: Identifica períodos de repetição para cada número (1-60)
...
- **Gaps**: Lacunas grandes entre números (ajustado para faixa 1-60)
- **Distribuição por Faixas**: Análise de números por grupos (1-10, 11-20, ..., 51-60)

### 3. **Padrões de Grupos**
- **Co-ocorrências**: Pares e trios que aparecem frequentemente juntos
...
O sistema gera **6 números mais prováveis** usando:
...
## 📊 Diferenças em Relação à Lotofácil

### Parâmetros Ajustados
- **Números sorteados**: 6 (vs 15 na Lotofácil)
- **Faixa numérica**: 1-60 (vs 1-25 na Lotofácil)
- **Gaps grandes**: > 10 (vs > 3 na Lotofácil)
- **Ausência prolongada**: > 15 sorteios (vs > 5 na Lotofácil)
- **Distribuição por faixas**: 6 faixas de 10 números cada

### Análises Específicas
- Análise de pares e trios (não quartetos, pois são apenas 6 números)
- Intervalos maiores entre números são esperados
- Distribuição mais espalhada pelos 60 números
...
- Números com probabilidade > 0.02 são considerados mais prováveis (ajustado para 60 números)
- Ajustes temporais e de co-ocorrência podem aumentar/diminuir probabilidades

### Padrões Identificados
- **Ciclos**: Se um número tem ciclo de 10 sorteios, ele tende a aparecer a cada 10 sorteios
...
No arquivo `treino_avancado.py`, função `gerar_6_numeros_inteligentes`:
```python
...
1. Adicione novos sorteios ao arquivo `mega_sena.xlsx`
...
- Verifique se o arquivo `mega_sena.xlsx` existe
- Verifique se o formato está correto (6 números por linha)
...
- Isso pode levar alguns minutos (mais tempo que Lotofácil devido a 60 números)

### Performance lenta
- Reduza o número de candidatos gerados
- Use menos splits na validação temporal
- Considere usar menos features (60 números = mais features)
