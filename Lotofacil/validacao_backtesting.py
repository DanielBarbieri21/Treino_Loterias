"""
🎯 Sistema de Backtesting e Validação Real - Lotofácil
========================================================
...
def backtest_modelo(df_numeros, criar_features_func, gerar_numeros_func, 
    ...
    """
    print("🔍 Iniciando Backtesting Real...")
    ...
    for i in range(inicio, len(df_numeros)):
        ...
        try:
            ...
            # (Aqui você deve implementar sua lógica de treino e previsão)
            previsao = gerar_previsao_para_backtesting(
                dados_treino, 
                criar_features_func, 
                gerar_numeros_func
            )
...
def gerar_previsao_para_backtesting(dados_treino, criar_features_func, gerar_numeros_func):
    ...
    # Criar y_binario
    y_binario = pd.DataFrame(0, index=np.arange(len(dados_treino)), columns=range(1, 26))
    ...
    # Gerar previsão: usar probabilidades e pegar top 15
    ...
    # Pegar top 15 números com maior probabilidade
    top_15_indices = np.argsort(probs)[-15:]
    previsao = sorted([int(idx + 1) for idx in top_15_indices])
...
def calcular_estatisticas_backtesting(resultados):
    ...
    stats = {
        ...
        'acerto_aleatorio_esperado': 9.0,  # (15/25) * 15 = 9
        'melhoria_sobre_aleatorio': 0
    }
    
    # Distribuição de acertos
    for n_acertos in range(16):
        stats['distribuicao_acertos'][n_acertos] = acertos.count(n_acertos)
...
def visualizar_resultados_backtesting(resultados, salvar=True):
    ...
    ax1.bar(acertos_valores, acertos_frequencias, color='skyblue', edgecolor='black')
    ...
    # 3. Taxa de acerto por número
    ax3 = axes[1, 0]
    numeros = sorted(stats['taxa_acerto_por_numero'].keys())
    taxas = [stats['taxa_acerto_por_numero'][n]['taxa'] for n in numeros]
    colors = ['green' if t > 60 else 'orange' if t > 50 else 'red' for t in taxas]
    ax3.bar(numeros, taxas, color=colors, edgecolor='black', alpha=0.7)
    ax3.axhline(60, color='blue', linestyle='--', label='Esperado (60%)')
    ax3.set_xlabel('Número')
    ax3.set_ylabel('Taxa de Acerto (%)')
    ax3.set_title('Taxa de Acerto por Número')
    ax3.legend()
    ...
    • Desvio Padrão: {stats['std_acertos']:.2f}
    
    Comparação:
    • Aleatório esperado: {stats['acerto_aleatorio_esperado']:.2f}
    • Melhoria: {stats['melhoria_sobre_aleatorio']:.2f}%
    
    Taxa de Acerto Geral:
    • {(stats['media_acertos'] / 15 * 100):.2f}% dos números
    """
    ...
    if salvar:
        plt.savefig('backtesting_resultados_lotofacil.png', dpi=300, bbox_inches='tight')
        print("📊 Gráfico salvo: backtesting_resultados_lotofacil.png")
...
def gerar_relatorio_backtesting(resultados, salvar_txt=True):
    ...
    relatorio = f"""
{'='*80}
🎯 RELATÓRIO DE BACKTESTING - LOTOFÁCIL
{'='*80}
...
Comparação com Aleatório:
  ...
  • Melhoria sobre Aleatório:  {stats['melhoria_sobre_aleatorio']:.2f}%

Taxa de Acerto Geral: {(stats['media_acertos'] / 15 * 100):.2f}%
...
"""
    ...
    for n_acertos in sorted(stats['distribuicao_acertos'].keys()):
        ...
        if freq > 0:
            ...
            barra = '█' * int(percent / 2)
            relatorio += f"{n_acertos:2d} acertos: {freq:3d} vezes ({percent:5.1f}%) {barra}\n"
    
    relatorio += f"""
{'='*80}
🎯 TOP 10 NÚMEROS COM MELHOR TAXA DE ACERTO
{'='*80}
...
"""
    ...
    for i, (num, info) in enumerate(numeros_ordenados[:10], 1):
        relatorio += f"{i:2d}. Número {num:2d}: {info['taxa']:5.1f}% "
        relatorio += f"({info['acertos']}/{info['aparicoes']} aparições)\n"
    
    relatorio += f"""
{'='*80}
❌ TOP 10 NÚMEROS COM PIOR TAXA DE ACERTO
{'='*80}

"""
    
    for i, (num, info) in enumerate(numeros_ordenados[-10:][::-1], 1):
        relatorio += f"{i:2d}. Número {num:2d}: {info['taxa']:5.1f}% "
        relatorio += f"({info['acertos']}/{info['aparicoes']} aparições)\n"
...
"""
    ...
    for detalhe in resultados['acertos_detalhados'][-10:]:
        relatorio += f"Sorteio #{detalhe['sorteio_index']}:\n"
...
"""
    ...
    if stats['melhoria_sobre_aleatorio'] > 5:
        relatorio += "✅ POSITIVO: O modelo apresenta melhoria significativa sobre o aleatório.\n"
    elif stats['melhoria_sobre_aleatorio'] > 0:
        relatorio += "⚠️  MODERADO: O modelo apresenta leve melhoria sobre o aleatório.\n"
    else:
        relatorio += "❌ NEGATIVO: O modelo NÃO apresenta melhoria sobre o aleatório.\n"
    
    relatorio += f"\nMédia de Acertos: {stats['media_acertos']:.2f}/15 ({(stats['media_acertos']/15*100):.1f}%)\n"
    relatorio += f"Aleatório Esperado: {stats['acerto_aleatorio_esperado']:.2f}/15 (60%)\n"
    
    relatorio += f"\n{'='*80}\n"
    
    if salvar_txt:
        filename = f"relatorio_backtesting_lotofacil_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt"
